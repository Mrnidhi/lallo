...
# This reads saved status and timing; it does not ask an agent or change the checkpoint.
...
if not callable(globals().get("load_checkpoint")):
    ...
    print("Configured timeout (seconds):", saved.get("transport", {}).get("timeout_seconds"))
    saved_trials = saved["trials"]
    if not saved_trials:
