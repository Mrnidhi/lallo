...
MANIFEST = {
    ...
    "original_gold_version": 32,
    "endpoints": ENDPOINTS,
    "warehouse": WAREHOUSE_ID,
    "prompts": PROMPTS,
    "expected": GT,
    "ground_truth_sql": GT_SQL,
    "ground_truth_parameters": GT_PARAMETERS,
    "routing_contract": ROUTING_CONTRACT,
    "contracts": CONTRACTS,
    "traceability": TRACEABILITY,
    "fixtures": {"lookup": LOOKUP_FIXTURE, "sales": SALES_FIXTURES},
    "reference_sha256": DRAFT_REVIEW_SHA256,
    "repetitions": REPETITIONS,
    "reference_status": "CALCULATIONS_CROSS_CHECKED_NOT_BUSINESS_SIGNED_OFF",
    "session_timezone": SESSION_TIME_ZONE,
    "experiment_note": (
        "New routing-contract experiment. Earlier C01 client2 attempts remain in their "
        "original evidence file; they are not reconciled, copied or erased by this run."
    ),
    "limitations": [
        "Underlying managed models and equality are unverified.",
        "This is not a replica of the custom production supervisor.",
        ...
        "The supervisor routing contract was manually read back before this experiment; "
        "this notebook does not independently fetch the saved UI configuration.",
        "Agreement with reference SQL does not independently certify business policy.",
        "Only 12 of the 41 proposed questions are in this batch.",
        "Enrichment, threshold calibration and swap scoring remain deferred.",
    ],
}
...
for repetition in range(1, REPETITIONS + 1):
    for number, qid in enumerate(QUESTION_IDS):
        ...
        for arm in arms:
            PLAN.append(
                {
                    "trial_id": trial_id(qid, arm, repetition),
                    "question": qid,
                    "arm": arm,
...
assert len(PLAN) == len({item["trial_id"] for item in PLAN})
assert len(PLAN) == len(QUESTION_IDS) * len(ENDPOINTS) * REPETITIONS
...
def validate_evidence_path():
    ...
    assert expected_parent.is_dir(), (
        "Personal workspace files are unavailable. Stop; do not use another destination."
    )
...
def save_checkpoint(state):
    ...
    try:
        ...
        assert EVIDENCE_PATH.read_text(encoding="utf-8") == payload, (
            "Evidence read-back failed. Stop before another request."
        )
    finally:
...
try:
    if EVIDENCE_PATH.exists():
        ...
        assert STATE["manifest"] == MANIFEST, (
            "An existing checkpoint has different test settings. Keep it unchanged; "
