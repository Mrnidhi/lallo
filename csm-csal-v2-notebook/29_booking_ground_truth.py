# Notebook cell 4 (file 29) | Independent booking answers
# SQL and DataFrame results must agree before a business review can approve them.
...
CONTRACTS = {}
LOOKUP_FIXTURE = None
# BASE is the loaded DataFrame. SQL needs the table identifier, BASELINE.
assert isinstance(BASELINE, str) and BASELINE == "usr.jayarsr.src_sales_ai_assistant_gold_csm_csal_summary_freeze_poc_v2_v23"
assert isinstance(BASELINE_VERSION, int) and not isinstance(BASELINE_VERSION, bool) and BASELINE_VERSION >= 0
...
ratio = F.when(F.col("total_reviewed_teu") > 0, F.col("confirmed_teu").cast("double") / F.col("total_reviewed_teu") * 100)
base_scopes = base_scopes.withColumn("fulfillment_pct", ratio)
for source, target in [("booking_rate", "booking_pct"), ("cancellation_rate", "cancellation_pct"), ("rejection_rate", "rejection_pct")]:
...
def keep_answer(qid, sql, parameters, reference, view_answer, columns, precision):
    ...
    # Compare numeric values, not Decimal-versus-double storage representations.
    assert comparable_rows(sql_rows, precision) == comparable_rows(independent_rows, precision), qid + ": SQL and independent DataFrame answers differ."
    assert comparable_rows(independent_rows, precision) == comparable_rows(after_rows, precision), qid + ": before/after answers differ."
    ...
    print(qid, "draft expected rows:", len(sql_rows))
...
for qid, flag, metric, descending, values in cases:
    ...
    if flag:
        ...
        where_sql = " AND ".join(name + " IS NOT NULL AND trim(CAST(" + name + " AS STRING)) <> ''" for name in SCOPE)
        where_sql += " AND confirmed_teu IS NOT NULL AND total_reviewed_teu > 0"
    ordering = ([F.col(metric).desc_nulls_last()] if descending else [F.col(metric).asc_nulls_last()]) + scope_order
    ...
    keep_answer(qid, sql, {"week": FILTERS["week"], "service": FILTERS["service"]}, ref, aft, columns, precision)
    assert GT[qid]["answer"], qid + ": empty population is not a useful ranking test."

# Select an exact lookup before seeing an agent answer. Values remain here.
present = F.lit(True)
for name in BOOKING_MEASURES:
    present = present & F.col(name).isNotNull()
candidates = BASE.filter(scope_filter).filter(F.col("tcr") == FILTERS["tcr"]).filter(valid_keys).groupBy(*SCOPE).agg(
    F.count("*").alias("physical_rows"), F.countDistinct(F.struct(*BOOKING_MEASURES)).alias("versions"),
...
assert selected, "No independently verifiable repeated-row C06 fixture."
...
keep_answer("C06", lookup_sql, LOOKUP_FIXTURE, base_scopes.filter(condition), after_scopes.filter(condition), BOOKING_MEASURES, {name: None for name in BOOKING_MEASURES})
...
GT_PARAMETERS["C07"] = {"customer": MISSING_CUSTOMER, "week": FILTERS["week"], "service": FILTERS["service"]}
...
assert source_markers() == SOURCE_MARKER, "Source drift. Ground truth cannot be frozen."
preparation_stamp("booking", booking_payload())
print("C01-C07 independently checked at this snapshot. Business approval is still required.")
