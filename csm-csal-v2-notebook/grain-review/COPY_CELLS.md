...
Use one Python code block per cell in a personal Databricks notebook. There are now nine cells. If Cells 1–8 already completed, append and run only [Cell 9](#cell-9-inspect-missing-value-overlaps-and-swap-context); the first eight cells are unchanged. For a new session, run in order. Read [the interpretation guide](README.md) before using any result in the report.
