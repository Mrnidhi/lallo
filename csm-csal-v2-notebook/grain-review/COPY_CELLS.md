...
Use one Python code block per cell in a personal Databricks notebook. There are now ten cells. If Cells 1–9 already completed, append and run only [Cell 10](#cell-10-check-the-saved-monthly-and-mqc-rules); the first nine cells are unchanged. If you stopped after Cell 8, run Cell 9 before Cell 10. For a new session, run in order with your previously recorded source version. Read [the interpretation guide](README.md) before using any result in the report.
...
## Cell 9: Inspect missing-value overlaps and swap context

For a completed eight-cell review, append this block as Cell 9 and run only this cell. Leave Cells 1–8 unchanged. It uses the version already recorded by Cell 1. If the session restarted, rerun from Cell 1 with that recorded version set explicitly.
...
## Cell 10: Check the saved monthly and MQC rules

Append this cell after a successful Cell 9. It reads the same pinned month, simulates the saved monthly join using a key index from Gold, replays the saved MQC status expression and shows available writer references. It does not recover monthly amounts or confirm the deployed producer code. Do not change a production join based on this simulation.
